...
export default function SpotifyPlayer({ isConnected, onConnect, onMusicStateUpdate }: SpotifyPlayerProps) {
  ...
  // Handle music state updates from Socket.IO
  useEffect(() => {
    if (onMusicStateUpdate) {
      // This would be called from the parent component when Socket.IO events are received
      // For now, we'll refresh the playback state
      fetchPlaybackState();
    }
  }, [onMusicStateUpdate]);
  ...
  const handlePlayPause = async () => {
    ...
    setIsLoading(true);
    try {
      const endpoint = playbackState?.is_playing ? 'http://localhost:7777/spotify/pause' : 'http://localhost:7777/spotify/play';
  ...
  const handleSkipNext = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:7777/spotify/next', { method: 'POST' });
      const data = await response.json();
  ...
  const handleSkipPrevious = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:7777/spotify/previous', { method: 'POST' });
      const data = await response.json();
  ...
  const handleVolumeChange = async (newVolume: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:7777/spotify/volume?volume=${newVolume}`, { 
        method: 'POST' 
      });
      const data = await response.json();
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between h-full py-2">
          ...
          <div className="flex items-start justify-between">
            <div className="min-w-0 flex-1 pr-2">
              ...
              <p className="text-xs text-gray-400 truncate" title={track.artists[0]?.name}>
            ...
            <div className="flex items-center relative flex-shrink-0">
              ...
              {showVolumeSlider && (
                <div className="absolute -top-8 right-0 bg-gray-900 border border-gray-600 rounded p-1">
                  <input
                    type="range"
                    min="0"
                    max="100"
